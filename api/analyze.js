...
МОСКВА: DoorHan от 1890₽ (все типы), МосПанели от 1599₽ (ППС/ПИР), СП-Центр от 1680₽, ТеплоПремиум от 1850₽, МеталлПром от 1700₽, СП-Панель от 1720₽, ИЗОБУД от 1130₽ (с 1998г, ISO 9001), ГК ТехноСтиль от 1650₽ (все типы, заводы Тула+Калуга), Baustof от 1800₽, Stamet b2b от 1230₽, ТермоСпецПанель от 1750₽, Термопанель от 1800₽, СТРОЙПАНЕЛЬ от 2100₽, Энергосервис от 1650₽ (ППУ/ПИР)
...
Задача: проанализировать загруженную спецификацию/КП, сравнить цены с рынком, дать конкретные рекомендации.
...
module.exports = async (req, res) => {
  ...
  if (!fileBase64) {
    return res.status(400).json({ error: 'Загрузите файл спецификации' });
  }
  ...
  try {
    ...
    const userPrompt = `Клиент: ${name}, тел: ${phone}, email: ${email}
Файл: ${fileName || 'спецификация.xlsx'}

Данные из файла:
${tableText}

Проанализируй спецификацию и верни JSON строго в формате:
{
